...
def run():
    ...
    if is_first_run:
        ...
        cmd = 'emulator/emulator @{name} -gpu host -accel off -wipe-data -writable-system -verbose {custom_args}'.format(name=avd_name, custom_args=custom_args)
    else:
        logger.info('Using previously initialized AVD...')
        cmd = 'emulator/emulator @{name} -gpu host -accel off -verbose -writable-system {custom_args}'.format(name=avd_name, custom_args=custom_args)
