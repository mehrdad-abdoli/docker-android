...
def run():
    ...
    if is_first_run:
        ...
        cmd = 'emulator/emulator @{name} -gpu swiftshader_indirect -accel on -no-audio -verbose -wipe-data -writable-system -skip-adb-auth {custom_args}'.format(name=avd_name, custom_args=custom_args)
    else:
        logger.info('Using previously initialized AVD...')
        cmd = 'emulator/emulator @{name} -gpu swiftshader_indirect -accel on -no-audio -verbose -writable-system -skip-adb-auth {custom_args}'.format(name=avd_name, custom_args=custom_args)
